...
class GPTSoVITSGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        #创建一个文本输入框 (QLineEdit)，用于显示和管理文件路径。
        self.GPT_model_input = QLineEdit()
        #在文本框为空时显示的提示文本。在用户选择文件之前，显示“选择person”以指导用户操作。
        self.GPT_model_input.setPlaceholderText("选择person")
        #设置文本框的初始文本为 self.GPT_Path。self.GPT_Path 是一个字符串，通常包含文件的路径。这样，用户在选择文件之前，文本框会显示当前的文件路径（如果有的话）。
        self.GPT_model_input.setText(self.person_path)
        #设置文本框为只读模式，这样用户不能直接编辑文本框中的内容。这通常用于显示信息而不允许修改。
        self.GPT_model_input.setReadOnly(True)
        ...
        #input_layout.addWidget(self.person_label, 7, 0)
        input_layout.addWidget(self.person_combobox, 8, 0, 1, 1)
      

     

        #input_layout.addWidget(self.ref_language_label, 9, 0)
        input_layout.addWidget(self.voice_combobox, 10, 0, 1, 1)

       


        #input_layout.addWidget(self.ref_language_label, 11, 0)
        input_layout.addWidget(self.motion_combobox, 12, 0, 1, 1)
  
        input_layout.addWidget(self.choose_folder_button, 6, 2)
